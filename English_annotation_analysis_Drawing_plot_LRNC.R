...
incident_CVD_5_check_1 <- subset(incident_CVD_5_check_1, LRNC_all == "no_LRNC")####At baseline, participants without Lipid form the base population for this analysis.
...
####Check population/group proportions----
...
fig1_0 <- plot_ly() %>%
  add_markers(data = fit_rg2, 
              ...
              error_z = list(type = "data", 
                             ...
                             color = color_point,
                             thickness = 5,#Since this is a 3D plot, thickness has no effect here; consider adding lines manually for more DIY control.
                             width = 5)) %>% 
  layout(
    scene = list(
...
######################guideline cutoff-------------------
#METh_modvig_guideline tertiles
cutvalue <- incident_followup %>%
