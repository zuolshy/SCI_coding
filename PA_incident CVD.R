...
incident_CVD_5$death_check[incident_CVD_5$death_date > as.Date("2020-12-31")] <- 0#在随访中止之后，不算死亡
incident_CVD_5$death_time <- time_length(difftime(incident_CVD_5$death_date, incident_CVD_5$ergocar_date), "years")
incident_CVD_5$death_check[incident_CVD_5$death_date > incident_CVD_5$inc_stroke_date] <- 0#在stroke之后，不算死亡
...
# incident_CVD_5 <- subset(incident_CVD_5, !(!(inc_stroke == 0)&stroke_time < 1))#reverse causation
...
meth[c("ergoid", "ergocar_date", "ergocarfup_date", "METh_modvig", "income", 
       "rs_cohort", "FFQ_age", "alcohol", "CRP", "ethnicity", "stroke_time", "CVD_his", "stroke_subtype")] <- "" #variables not being imputed will also not be used as predictors
# predictor matrix
pred <- imp0$predictorMatrix
pred[ ,c("ergoid", "ergocar_date", "ergocarfup_date", "METh_modvig", "income",
         "rs_cohort", "FFQ_age", "alcohol", "CRP", "ethnicity", "stroke_time", "CVD_his", "stroke_subtype")] <- 0 #will not be used as predictor in any model
# incident_CVD_5_0 <- mice(incident_CVD_5, m=3, maxit=1, meth=meth, seed=619, predictorMatrix = pred)
...
summary(incident_CVD_5_long_imputed30)#确定是30轮插补的数据
...
incident_CVD_5_check_1 <- left_join(incident_CVD_5_check_1, ID_follow, by = "ergoid")#ID_follow是对斑块成份新发分析的结果，需要先去运行“手动基线分位”文件
...
# ####这里是对所有进入随访的人weighting，但这个并不是新发斑块的研究设计
# ####因为纳入最后分析的人是基线没有对应斑块成份的人
# ####所以应该是没有对应斑块的人，最后纳入到斑块新发里面的IPW才是所需要的权重
# ####对应的IPW应该在IPH以及LRNC新发的ID获得之后才能计算
# incident_CVD_5_check_1 <- subset(incident_CVD_5_check, .imp == 1)#response变量就是follow，0为失访，1为随访
# dput(names(incident_CVD_5_check_1))
# incident_CVD_5_check_1$Sex <- factor(incident_CVD_5_check_1$Sex, labels = c("Man", "Woman"))
# incident_CVD_5_check_1 <- left_join(incident_CVD_5_check_1, ID_follow, by = "ergoid")#ID_follow是对斑块成份新发分析的结果，需要先去运行“手动基线分位”文件
# incident_CVD_5_check_1$follow[is.na(incident_CVD_5_check_1$follow)] <- 0
# # propensity_score_model <- glm(follow ~  Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + hyperchol + hypertension_1 + dm + Max_IMT_complete,
# #                  family = binomial, data = incident_CVD_5_check_1)
# propensity_score_model <- glm(follow ~  Sex + edu_2 + hypertension_1 + dm,
#                               family = binomial, data = incident_CVD_5_check_1)
# 
# #获取原始的weighing
...
#   IPW = 1/ps)
# 
# summary(incident_CVD_5_check_1$ps)
# summary(incident_CVD_5_check_1$IPW_stabilized)
# 
# dput(names(incident_CVD_5_check_1))
# IPW_final <- subset(incident_CVD_5_check_1, select = c("ergoid", "IPW_stabilized", "IPW"))
# 
# ##############################IPH-------------------------------
# #####total PA----
# #####stroke
...
# full_cohort
# #no_IPH
# rg1 <- with(incident_CVD_5_long_no_IPH,
#             coxph(Surv(stroke_time, stat == 1) ~
#                     I(total_MET/20) +
#                     Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                     hyperchol + hypertension_1 + dm + Max_IMT_complete))
# summary(pool(rg1))
# no_IPH <- format_pooled_results(summary(pool(rg1))[1,])
# no_IPH
# #IPH
# rg1 <- with(incident_CVD_5_long_IPH,
#             coxph(Surv(stroke_time, stat == 1) ~
#                     I(total_MET/20) +
#                     Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                     hyperchol + hypertension_1 + dm + Max_IMT_complete))
# summary(pool(rg1))
# IPH <- format_pooled_results(summary(pool(rg1))[1,])
# IPH
# #interaction
# rg1 <- with(incident_CVD_5_long,
#             coxph(Surv(stroke_time, stat == 1) ~
#                     I(total_MET/20)*IPH_all +
#                     Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                     hyperchol + hypertension_1 + dm + Max_IMT_complete))
# summary(pool(rg1))
# fit_rg0 <- summary(pool(rg1))[14,]
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# total_stroke <- data.frame(full_cohort, no_IPH, IPH, p_interaction)
# total_stroke
# 
# 
# #####death
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# total_death <- data.frame(full_cohort, no_IPH, IPH, p_interaction)
# total_death
# 
# 
# #####modvig PA----
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# modvig_stroke <- data.frame(full_cohort, no_IPH, IPH, p_interaction)
# modvig_stroke
# 
# 
# #####death
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# modvig_death <- data.frame(full_cohort, no_IPH, IPH, p_interaction)
# modvig_death
# 
# 
# #####moderate PA----
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# moderate_stroke <- data.frame(full_cohort, no_IPH, IPH, p_interaction)
# moderate_stroke
# 
# 
# #####death
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# moderate_death <- data.frame(full_cohort, no_IPH, IPH, p_interaction)
# moderate_death
# 
# 
# #####vigorous PA----
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# vigorous_stroke <- data.frame(full_cohort, no_IPH, IPH, p_interaction)
# vigorous_stroke
# 
# 
# #####death
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# vigorous_death <- data.frame(full_cohort, no_IPH, IPH, p_interaction)
# vigorous_death
# 
# 
# final_IPH <- rbind(total_stroke,total_death,modvig_stroke,modvig_death,
#                    moderate_stroke,moderate_death,vigorous_stroke,vigorous_death)
# final_IPH
# 
# ####nonliear IPH----
# #stroke
...
# #moderate
# rg1 <- coxph(Surv(stroke_time, stat == 1) ~
#                METh_moderate + IPH_all + 
#                Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                hyperchol + hypertension_1 + dm + Max_IMT_complete, data = data_CVD_nonlinear)
# rg2 <- coxph(Surv(stroke_time, stat == 1) ~
#                ns(METh_moderate,  3) + IPH_all + 
#                Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                hyperchol + hypertension_1 + dm + Max_IMT_complete, data = data_CVD_nonlinear)
# print(anova(rg1, rg2), digits = 4)
# 
# #vigorous
# rg1 <- coxph(Surv(stroke_time, stat == 1) ~
#                METh_vigorous + IPH_all + 
#                Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                hyperchol + hypertension_1 + dm + Max_IMT_complete, data = data_CVD_nonlinear)
# rg2 <- coxph(Surv(stroke_time, stat == 1) ~
#                ns(METh_vigorous,  knots = c(1,5,10)) + IPH_all + 
#                Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                hyperchol + hypertension_1 + dm + Max_IMT_complete, data = data_CVD_nonlinear)
# print(anova(rg1, rg2), digits = 4)
# 
...
# #death
# #total
# rg1 <- coxph(Surv(stroke_time, stat == 2) ~
#                total_MET +  IPH_all + 
#                Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                hyperchol + hypertension_1 + dm + Max_IMT_complete, data = data_CVD_nonlinear)
# rg2 <- coxph(Surv(stroke_time, stat == 2) ~
#                ns(total_MET,  3) + IPH_all + 
#                Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                hyperchol + hypertension_1 + dm + Max_IMT_complete, data = data_CVD_nonlinear)
# print(anova(rg1, rg2), digits = 4)
...
# full_cohort
# #no_LRNC
# rg1 <- with(incident_CVD_5_long_no_LRNC,
#             coxph(Surv(stroke_time, stat == 1) ~
#                     I(total_MET/20) +
#                     Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                     hyperchol + hypertension_1 + dm + Max_IMT_complete))
# summary(pool(rg1))
# no_LRNC <- format_pooled_results(summary(pool(rg1))[1,])
# no_LRNC
# #LRNC
# rg1 <- with(incident_CVD_5_long_LRNC,
#             coxph(Surv(stroke_time, stat == 1) ~
#                     I(total_MET/20) +
#                     Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                     hyperchol + hypertension_1 + dm + Max_IMT_complete))
# summary(pool(rg1))
# LRNC <- format_pooled_results(summary(pool(rg1))[1,])
# LRNC
# #interaction
# rg1 <- with(incident_CVD_5_long,
#             coxph(Surv(stroke_time, stat == 1) ~
#                     I(total_MET/20)*LRNC_all +
#                     Sex + First_age + BMI + smokingstatus_2 + rs_cohort + edu_2 + 
#                     hyperchol + hypertension_1 + dm + Max_IMT_complete))
# summary(pool(rg1))
# fit_rg0 <- summary(pool(rg1))[14,]
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# total_stroke <- data.frame(full_cohort, no_LRNC, LRNC, p_interaction)
# total_stroke
# 
# 
# #####death
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# total_death <- data.frame(full_cohort, no_LRNC, LRNC, p_interaction)
# total_death
# 
# 
# 
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# modvig_stroke <- data.frame(full_cohort, no_LRNC, LRNC, p_interaction)
# modvig_stroke
# 
# 
# #####death
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# modvig_death <- data.frame(full_cohort, no_LRNC, LRNC, p_interaction)
# modvig_death
# 
# #####moderate PA----
# #####stroke
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# moderate_stroke <- data.frame(full_cohort, no_LRNC, LRNC, p_interaction)
# moderate_stroke
# 
# 
# #####death
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# moderate_death <- data.frame(full_cohort, no_LRNC, LRNC, p_interaction)
# moderate_death
# 
# #####vigorous PA----
# #####stroke
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# vigorous_stroke <- data.frame(full_cohort, no_LRNC, LRNC, p_interaction)
# vigorous_stroke
# 
# 
# #####death
...
# p_interaction <- c(round(fit_rg0$p.value, digits = 4))
# p_interaction
# vigorous_death <- data.frame(full_cohort, no_LRNC, LRNC, p_interaction)
# vigorous_death
# 
# final_LRNC <- rbind(total_stroke,total_death,modvig_stroke,modvig_death,
#                    moderate_stroke,moderate_death,vigorous_stroke,vigorous_death)
# final_LRNC
# 
# 
